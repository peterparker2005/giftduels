...
type IdentityPublicHandler struct {
	identityv1.UnimplementedIdentityPublicServiceServer
	tokenSvc token.TokenService
	logger   *zap.Logger
	cfg      *config.Config
}

func NewIdentityPublicHandler(tokenSvc token.TokenService, logger *zap.Logger, cfg *config.Config) *IdentityPublicHandler {
